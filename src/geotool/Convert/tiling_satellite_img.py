...
# %%
#### 今貼ってあるのはgeotiff ⇒ GPKG Tilesに変換するコード

def tiling_satellite_img(src_path, dst_path, band_ls = [0, 1, 2], vmins=[None, None, None], vmaxes=[None, None,None], log=False):
    """画像をuint8に変換するコード

    Args:
        src_path (str: path): 変換元のパス
        band_ls (list: int): RGBのバンドのリスト
        vmins (list: float): 下限値のリスト (R G Bの順)
        vmaxes (list: float): 上限値のリスト (R G Bの順)
